...
class BossSpiderSpider(CrawlSpider):
    ...
    # https: // www.zhipin.com / c100010000 /?query = python & page = 3
    ...
    def parse_job(self, response):
        print(response.text)
        name = response.xpath('//div[@class="info-primary"]/div/h1/text()').extract_first().strip()
        salary = response.xpath('//div[@class="info-primary"]/div/span/text()').extract().strip()
        infos = response.xpath('//div[contains(@class,"job-primary")]/div[@class="info-primary"]/p/text()').extract()
        city = infos[0]
        work_years = infos[1]
        education = infos[2]
        company = response.xpath(
            '//div[@class="sider-company"]//div[@class="company-info"]/a[1]/@title').extract().strip()
        item = BossItem(name=name, salary=salary, city=city, work_years=work_years, education=education,
                        company=company)
        print('='*60)
